...
To ensure existing `admin.py` files are emptied before copying the new ones, you can first truncate (or delete) them, then copy. Here's a safe approach:

### Option 1: Truncate (empty in place) then copy

```bash
for f in docs/master_backend/admin/*_admin.py; do
    app=$(basename "$f" _admin.py)
    > "$app/admin.py"          # empty the file
    cp "$f" "$app/admin.py"    # copy new content
done
```

### Option 2: Delete then copy (cleaner)

```bash
for f in docs/master_backend/admin/*_admin.py; do
    app=$(basename "$f" _admin.py)
    rm -f "$app/admin.py"      # remove if exists
    cp "$f" "$app/admin.py"    # copy new file
done
```

Both will ensure no leftover content remains from previous `admin.py` files. The second option is more common.
