...
from core.deletion import describe_delete_plan
...
def raw_materials_list(request):
    ...
    if request.user.userprofile.role == "manager":
        # Only computed for the role that can actually see/use the trash
        # icon — describe_delete_plan() does real queries per row, no
        # need to pay that cost for roles that never see the button.
        for m in materials:
            plan = describe_delete_plan(m)
            m.can_delete = not plan["blocked"] and not plan["cascades"]
            if not m.can_delete:
                reasons = plan["blockers"] or plan["cascades"]
                m.delete_block_reason = ", ".join(
                    f"{count} {label}" for label, count in reasons
                )
...
def raw_material_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            material.save()
            form.save_m2m()  # BR-DUAL-01: creates/links the twin FinishedProduct if requested
...
def raw_material_edit(request, material_id):
    ...
    if request.method == "POST":
        form = RawMaterialForm(request.POST, instance=material, user=request.user)
        if form.is_valid():
            old_values = {
        ...
        form = RawMaterialForm(instance=material, user=request.user)
...
@login_required
@role_required(["manager"])
def raw_material_toggle_active(request, material_id):
    """Activate/deactivate a raw material — manager only"""
    if request.method == "POST":
        material = get_object_or_404(RawMaterial, id=material_id)
        old_status = material.is_active
        ...
        AuditLog.log_action(
            ...
            details={
                ...
                "before": old_status,
                "after": material.is_active,
            },
            request=request,
        )
        status_label = "activée" if material.is_active else "désactivée"
        messages.success(
            request, f"Matière première {material.reference} {status_label}"
        )
    return redirect("catalog:raw_material_detail", material_id=material_id)
...
def raw_material_delete(request, material_id):
    """Hard delete — manager only, and only when nothing else references
    this material anywhere in the app (see core.deletion.can_hard_delete).
    Everywhere else, deactivation (raw_material_toggle_active above) is the
    correct action — this exists for cleaning up records created by
    mistake, not for retiring materials that have real history."""
    material = get_object_or_404(RawMaterial, id=material_id)
    plan = describe_delete_plan(material)
    if plan["blocked"] or plan["cascades"]:
        reasons = plan["blockers"] or plan["cascades"]
        detail = ", ".join(f"{count} {label}" for label, count in reasons)
        messages.error(
            request,
            f"Impossible de supprimer « {material.designation} » : utilisée dans {detail}. "
            "Désactivez-la plutôt.",
        )
        return redirect("catalog:raw_material_detail", material_id=material_id)

    reference, designation = material.reference, material.designation
    AuditLog.log_action(
        user=request.user, action_type="delete", module="catalog", instance=material,
        details={"reference": reference, "designation": designation}, request=request,
    )
    material.delete()
    messages.success(request, f"Matière première {reference} — {designation} supprimée.")
    return redirect("catalog:raw_materials_list")
...
def finished_products_list(request):
    ...
    if request.user.userprofile.role == "manager":
        for p in products:
            plan = describe_delete_plan(p)
            p.can_delete = not plan["blocked"] and not plan["cascades"]
            if not p.can_delete:
                reasons = plan["blockers"] or plan["cascades"]
                p.delete_block_reason = ", ".join(
                    f"{count} {label}" for label, count in reasons
                )
...
def finished_product_create(request):
    if request.method == "POST":
        form = FinishedProductForm(request.POST, user=request.user)
        if form.is_valid():
            product = form.save(commit=False)
            product.created_by = request.user
            product.save()
            form.save_m2m()  # BR-DUAL-01: creates/links the twin RawMaterial if requested
            AuditLog.log_action(
                user=request.user,
                action_type="create",
                module="catalog",
        ...
        form = FinishedProductForm(user=request.user)
...
def finished_product_edit(request, product_id):
    ...
    if request.method == "POST":
        form = FinishedProductForm(request.POST, instance=product, user=request.user)
        if form.is_valid():
        ...
        form = FinishedProductForm(instance=product, user=request.user)
...
@role_required(["manager"])
@require_POST
def finished_product_delete(request, product_id):
    """Hard delete — manager only, and only when unreferenced anywhere
    (see raw_material_delete above for the same reasoning)."""
    product = get_object_or_404(FinishedProduct, pk=product_id)
    plan = describe_delete_plan(product)
    if plan["blocked"] or plan["cascades"]:
        reasons = plan["blockers"] or plan["cascades"]
        detail = ", ".join(f"{count} {label}" for label, count in reasons)
        messages.error(
            request,
            f"Impossible de supprimer « {product.designation} » : utilisé dans {detail}. "
            "Désactivez-le plutôt.",
        )
        return redirect("catalog:finished_product_detail", product_id=product_id)

    reference, designation = product.reference, product.designation
    AuditLog.log_action(
        user=request.user, action_type="delete", module="catalog", instance=product,
        details={"reference": reference, "designation": designation}, request=request,
    )
    product.delete()
    messages.success(request, f"Produit fini {reference} — {designation} supprimé.")
    return redirect("catalog:finished_products_list")
