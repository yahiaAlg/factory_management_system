...
class Expense(models.Model):
    ...
    # project; replaces the ad-hoc SupportingDocument entity_type/entity_id
    # lookup used previously for the SD-EXP gate below.
    pieces_jointes = GenericRelation(PieceJointe, related_query_name="expense")
    ...
    def validate(self, user):
        ...
        if self.amount > threshold:
            ...
            if not self.pieces_jointes.filter(
                type_document=PieceJointe.TYPE_SD_EXP
            ).exists():
                raise ValidationError(
    ...
    def mark_as_paid(self, user, payment_date, payment_method, bank_reference=""):
