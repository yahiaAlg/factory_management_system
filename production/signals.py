...
def create_fg_movement_on_po_completion(sender, instance, created, **kwargs):
    ...
    if not instance.actual_qty_produced or instance.actual_qty_produced <= 0:
        return
    # QA/QC Gate C (functional spec §6.5): a "Scrap" NCR disposition writes the
    # batch off instead of crediting sellable finished-goods stock.
    if instance.scrapped:
        return
...
def create_rm_consumption_on_line_save(sender, instance, created, **kwargs):
    ...
    # BR-QA-06: raw material is physically consumed regardless of the Gate C
    # QC/yield outcome, so deduction fires as soon as actual consumption is
    # recorded — not only once the order reaches a final "completed" state.
    if po.status not in ("completed", "pending_qc_release", "completed_investigation"):
        return
