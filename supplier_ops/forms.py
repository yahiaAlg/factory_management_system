...
class SupplierDNForm(forms.ModelForm):
    class Meta:
        model = SupplierDN
        fields = [
            'external_reference', 'supplier', 'delivery_date', 'remarks'
        ]
        widgets = {
...
class SupplierDNLineForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'raw_material', 'quantity_received', 'unit_of_measure', 'agreed_unit_price'
        ]
        widgets = {
            'quantity_received': forms.NumberInput(attrs={'step': '0.001', 'min': '0.001'}),
...
class SupplierInvoiceForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'external_reference', 'supplier', 'invoice_date', 'due_date'
        ]
        widgets = {
...
class SupplierInvoiceLineForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'raw_material', 'designation', 'quantity_invoiced', 'unit_price_invoiced'
        ]
        widgets = {
            'designation': forms.TextInput(attrs={'size': 40}),
            'quantity_invoiced': forms.NumberInput(attrs={'step': '0.001', 'min': '0.001'}),
            'unit_price_invoiced': forms.NumberInput(attrs={'step': '0.01', 'min': '0'}),
        }
