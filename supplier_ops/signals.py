...
def update_stock_on_dn_validation(sender, instance, created, **kwargs):
    ...
    for line in instance.lines.all():
        # BR-QA-02 (hard invariant): a Gate-A-flagged line whose latest sample
        # is Non-Conforming never credits stock, even for an otherwise-validated
        # DN (partial validation — functional spec §4.2 Step 5c).
        latest_sample = line.quality_samples.order_by("-sampled_at").first()
        if latest_sample and latest_sample.status == "non_conforming":
            continue
