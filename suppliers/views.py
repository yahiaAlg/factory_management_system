...
def suppliers_list(request):
    ...
    if request.user.userprofile.role == "manager":
        for s in suppliers:
            plan = describe_delete_plan(s)
            s.can_delete = not plan["blocked"] and not plan["cascades"]
            if not s.can_delete:
                reasons = plan["blockers"] or plan["cascades"]
                s.delete_block_reason = ", ".join(
...
def supplier_detail(request, supplier_id):
    ...
    from supplier_ops.models import SupplierAdvance

    supplier = get_object_or_404(Supplier, id=supplier_id)
    current_year = timezone.now().year

    # §23.3 (planned) — advances still available for this supplier.
    available_advances = SupplierAdvance.objects.filter(
        supplier=supplier, remaining_amount__gt=0
    ).order_by("date")
    available_advance_total = available_advances.aggregate(
...
def supplier_delete(request, supplier_id):
    ...
    (no BL, no invoice, no advance, nothing pointing at this supplier)."""
    supplier = get_object_or_404(Supplier, id=supplier_id)
    if request.method == "POST":
        plan = describe_delete_plan(supplier)
        if plan["blocked"] or plan["cascades"]:
            reasons = plan["blockers"] or plan["cascades"]
            detail = ", ".join(f"{count} {label}" for label, count in reasons)
            messages.error(
                request,
                f"Impossible de supprimer « {supplier.raison_sociale} » : utilisé dans {detail}. "
                "Désactivez-le plutôt.",
            )
            return redirect("suppliers:supplier_detail", supplier_id=supplier_id)

        code, raison_sociale = supplier.code, supplier.raison_sociale
        AuditLog.log_action(
            user=request.user, action_type="delete", module="suppliers", instance=supplier,
            details={"code": code, "raison_sociale": raison_sociale}, request=request,
        )
        supplier.delete()
        messages.success(request, f"Fournisseur {code} — {raison_sociale} supprimé.")
        return redirect("suppliers:suppliers_list")
    return redirect("suppliers:supplier_detail", supplier_id=supplier_id)
